...
export const submit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (issue.status === "in_progress") {
      await ctx.db.patch(issue._id, { status: "pending_verification", updatedAt: Date.now() });
...
export const verify = mutation({
  args: { evidenceId: v.id("resolutionEvidence") },
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(evidence._id, { verifiedBy: actor._id, verifiedAt: Date.now() });
    await audit(ctx, actor._id, "evidence.verify", "resolutionEvidence", evidence._id, { issueId: evidence.issueId });
  },
});
